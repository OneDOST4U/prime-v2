...
| **Status (2026-07-13, Phase 14–15 investigation)** | **Still open — re-confirmed.** Ran the full suite with parallel file execution re-enabled (`vitest --run`, no `--no-file-parallelism`): all 132 test assertions passed, but `rd.test.ts`'s `afterAll` cleanup threw `PrismaClientKnownRequestError` — FK constraint violation on `proposal_versions_form_template_version_id_fkey` deleting a `formTemplateVersion` still referenced by another file's in-flight proposal data. Confirms the root cause is shared-fixture/teardown-ordering collisions across files, not flaky assertions. A real fix (per-worker DB or strict per-file namespacing of ALL shared rows, not just user emails) is a bigger lift than fits this pass — keeping `--no-file-parallelism` in `package.json` per the existing contingency. Do not re-enable parallel test execution. |
